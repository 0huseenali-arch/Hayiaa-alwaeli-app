...
// ---------------------------------------------------------------
// جعفرية 3 أوقات — زاوية الفجر 18° (مطابقة لتطبيق حقيبة المؤمن)
const FAJR_ANGLE = 18;
...
exports.onNewPost = onValueWritten(
  { ref: "/hayaa_posts", instance: "alwaeli-e5bf0-default-rtdb", region: "asia-southeast1" },
  async (event) => {
    try {
      ...
      if (!newest || beforeIds.has(newest.id)) {
        console.log("onNewPost: المنشور الأحدث موجود مسبقاً بالقائمة القديمة، لا إرسال.", newest && newest.id);
        return;
      }
      ...
      const response = await messaging.send({
        condition: "'pref_newPost' in topics",
        notification: { title: "منشور جديد من الهيئة", body: caption },
        data: { type: "newPost", postId: String(newest.id) },
        webpush: { fcmOptions: { link: "https://hayiaa-alwaeli-app.onrender.com/" } },
      });
      console.log("onNewPost: نجح الإرسال، messageId:", response);
    } catch (err) {
...
exports.updateNotificationSubscriptions = onCall(
  ...
  async (request) => {
    ...
    for (const gov of GOVERNORATES) {
      ...
      if (gov.slug === governorate) {
        try {
          ...
          subscribeResults[topic] = "ok";
        } catch (err) {
          subscribeResults[topic] = "فشل: " + err.message;
          console.error(`فشل الاشتراك بموضوع ${topic}:`, err.message);
        }
      } else {
    ...
    console.log("نتيجة الاشتراكات:", JSON.stringify(subscribeResults));
    ...
    return { success: true, subscribeResults };
  }
);
